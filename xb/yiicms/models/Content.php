...
class Content extends AppActiveRecord
{
    ...
    public function getCategorytree()
    {
        ...
        $category_array = explode("/",$obj->path);
        $category_ids = implode(',',$category_array);
        \yii::info(var_export($category_ids,true));
        $my=Category::find()
            ->select('id,name')
            ->where(['IN','id',$category_array])
            ->all();
        \yii::info($my);
        $temparray =$category_array;
        foreach ($category_array as $key => $value) {
            foreach ($my as $key1 => $value1) {
                if ($value1->id== $value) {
                   $temparray = str_replace($value,$value1->name,$temparray); 
                   continue; 
                }                
            }            
        }
        \yii::info(var_export($temparray,true));
        array_push($temparray, $obj->name);
        $resobj->name = implode('/',$temparray);
        return $resobj;
